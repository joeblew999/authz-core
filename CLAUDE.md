...
- **Biz validation** (`mise run validate:biz`): **6/7 PASS for v1 (EVENT + PLATFORM).** The single failure is `owner_cannot_manage_divisions_on_camp` — exactly the predicted event-subtype-scoping caveat. **Resolution is biz-side, not engine-side**: split `EVENT` into `tournament`/`league`/`showcase`/`camp` types so `manage_divisions` only exists on the first three. No CEL needed.
